...
Po pierwszej testówce Jakub zaznaczył na zrzucie czerwonym markerem, którędy ma biec pomiar — chodziło o przebieg linii, nie o kolor; czerwień to było zaznaczenie, a same linie mają zostać białe. Trzy krawędzie niosące wymiary są więc grubsze i białe: głębokość stosu, długość po dolnej krawędzi i wysokość po prawej pionowej. Krawędź głębokości film urywa po 21 px, bo dalej zasłaniają ją kłody — prowadzimy ją do pełnej długości w kierunku odczytanym z tego kikuta, na jedną piątą dolnej krawędzi, czyli w proporcji 2,00 m do 10,00 m. Dopiero wtedy z dołu robi się czytelne „V”, o które chodziło. Kamera odjeżdża, więc współrzędne są wykrywane osobno w każdej klatce (`scratchpad/apple/geometria.py`): krawędzie pionowe to kolumny z największą liczbą jasnych, odbarwionych pikseli, dolna krawędź to prosta dopasowana do najniższych jasnych pikseli z odrzucaniem odstających. Bez tego czerwone linie odklejałyby się od stosu w trakcie pętli — bryła zwęża się z 709 do 682 px przez cztery klatki. Ścieżka w mailu dostała `?v=2`, bo proxy Gmaila trzyma obrazki pod starym adresem.
