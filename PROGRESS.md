...
**Animowany przycisk w cenniku (09.09).** Zamiast płaskiego guzika `PrzyciskOferty` w `ProductPage.tsx`: pigułka z przesuwającym się gradientem zieleń → czerń i płynącymi poziomicami — tym samym wzorem (`ContourTexture`), co na leśnym panelu „Proces zakupu i wdrożenia”. Droga do tego: najpierw siedem bladych drobinek („to nie są drobinki”), potem dwadzieścia dwie z poświatą, wygaszone o 20%, aż Jakub zaproponował poziomice zamiast kropek. Tor to dwie kopie tekstury w kontenerze o podwójnej szerokości, przesuwane o połowę (ten sam chwyt co w karuzeli logotypów, więc pętla jest niewidoczna), a do tego powolny „oddech” skalą. Drobinki i ich pozycje usunięte z komponentu (wzór z przycisku, który przysłał Jakub, przemalowany na naszą paletę; bez białego kółka na końcu — odrzucone). Animacje w `globals.css` (`oferta-gradient`, `oferta-drobinka`), w komponencie zostają tylko pozycje i opóźnienia drobinek, żeby ruch nie był zsynchronizowany. **Pułapka:** przy `background-size: 200%` i pętli o pełną szerokość tła pierwszy i ostatni przystanek gradientu muszą mieć ten sam kolor, inaczej na styku powtórzenia widać pionowy szew (widoczny na pierwszym podejściu); z tego samego powodu kąt jest prosty, a nie ukośny. `prefers-reduced-motion` zatrzymuje obie animacje.
