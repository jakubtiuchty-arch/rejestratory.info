...
**Samsung wstrzymany** (13.09, decyzja Jakuba). Wydanie z 15.09 nie miało wpisu w `newsletter_sends` ani nic w Resend — czekało wyłącznie na kliknięcie w poniedziałkowej testówce. `manifest.json` przełączony z Samsunga na wydanie HP, więc jutrzejsza testówka pokaże HP, a przycisk odpowie „Już zatwierdzone". Plik wydania Samsunga zostaje w repo do przełożenia.
...
**Hero — dwa podejścia.** Pierwsze: produktowa scena z trzema telefonami w nocnym lesie (gpt_image_2 z renderami jako referencją) plus najazd kamery. Jakub: hero ma być **wideo w klimacie poprzednich wydań**, czyli scena z pracy, a nie ustawka produktowa. Drugie i obowiązujące: leśniczy w porannym lesie ogląda XCover7 (obraz z gpt_image_2 z renderem telefonu jako referencją, prompt bez naszywek na mundurze), a ruch dogrywa **seedance_2_5 w trybie `omni_reference`** — 5 s, 1080p, 21:9, bez dźwięku, prompt pilnuje, żeby postać stała nieruchomo, a poruszały się mgła, promienie i paprocie. Z filmu wycięty pas 2,4:1, siedem klatek w pętli tam i z powrotem, paleta 128 — **640 kB**. Wersje z 64 kolorami i ditheringiem `bayer` odpadły: waga spada do 480 kB, ale kolory wychodzą wyprane.

**Galeria po uwadze Jakuba:** pod nagłówkiem „Samsung Galaxy XCover7” stały trzy różne telefony — zostało samo zdjęcie modelu wiodącego, pozostałe modele są niżej w kaflach.
...
**Jak to działa w takma.** Nie ma tam samoobsługi — klient wysyła zapytanie, handlowiec wycenia w panelu, system renderuje PDF przez `@react-pdf/renderer` (`src/lib/quote-pdf/QuotePdf.tsx`, fonty DejaVu z public/, logo z URL) i wysyła Resendem z załącznikiem oraz linkiem tokenowym; numeracja `OFR-RRRR-NNNNNN` z Prismy, VAT 23% na sztywno, warunki „7 dni” / „2-5 dni roboczych”. Tu przeniesiony jest sam szablon i mechanika renderu, a przód jest samoobsługowy.

**Co powstało (bez commita — czeka na akceptację):**
- `lib/oferta-pdf/produkty.ts` — dane do oferty w GROSZACH (Pospay 2: 179 900, abonament 3 900/mies., prowizje VISA 0,69 / MC 0,79 / BLIK 0,39 %, 7 kroków wdrożenia, uwagi), warunki (ważność 30 dni, płatność „przelew, 14 dni od daty faktury”, realizacja „do uzgodnienia z nadleśnictwem”) i dane sprzedawcy z stopki (TAKMA Tadeusz Tiuchty, Poświęcka 1a, NIP 915-100-43-77). Kolejny produkt = kolejny wpis pod slugiem karty.
- `lib/oferta-pdf/OfertaPdf.tsx` — szablon react-pdf w leśnej palecie: pas nagłówka `#0A1B12` z białym logo TAKMA i limonkową linią `#A8F000`, tytuły sekcji w `#166534`, nagłówek tabeli `#14532d`, podsumowanie na ciemnej zieleni z „Razem brutto” w limonce, boksy abonamentu i prowizji, rząd siedmiu kroków wdrożenia z limonkowymi numerami, uwagi zamawiającego z limonkową krawędzią, stopka z pełnymi danymi firmy i numeracją stron. Własny formater kwot `zl()` (bez `toLocaleString` — okrojony ICU w Node). Mieści się na jednej stronie A4 — trzeba było ścisnąć pas nagłówka i odstępy, pierwsza wersja wylewała same uwagi na drugą stronę.
- `lib/oferta-pdf/render.ts` — rejestracja DejaVu (skopiowane z takma do `public/fonts/`) i logo **z systemu plików**, gdy plik jest pod ręką, inaczej z `https://www.rejestratory.info`; w `next.config.js` `experimental.outputFileTracingIncludes` dokłada te trzy pliki do bundla funkcji na Vercelu.
- `app/api/oferta-pdf/route.ts` — POST JSON (slug, nadleśnictwo, adres, NIP opcjonalnie z walidacją 10 cyfr, osoba, e-mail, telefon, ilość 1–50, uwagi, zgoda, honeypot `www`), limit 5 ofert / 10 min per IP (pamięć procesu), numer z tabeli `oferty_pdf` w Supabase (`OF-RRRR-00001` z `id`), a bez tabeli numer z czasu (`OF-2026-MTTZ6OHM`) i ostrzeżenie w logu. Zwraca PDF jako `attachment` + nagłówek `X-Oferta-Numer`; równolegle dwa maile przez Resend z załącznikiem: kopia dla klienta i powiadomienie dla `handlowy@` + `rejestratory@takma.com.pl` (nadpisywalne `OFERTA_PDF_HANDLOWY`, użyte do testu). Błąd maila nie odbiera pliku. Mail w szacie z newslettera (ciemny blok, logo, stopka), pola escapowane.
- `components/product/OfertaPdfModal.tsx` — okno w stylu „Jakie dokumenty?” (portal do body, Escape, `ContourTexture`, limonkowy przycisk), etykiety nad polami, „(opcjonalnie)” zamiast gwiazdek, po sukcesie ekran z numerem oferty i adresem, na który poszła kopia; plik pobiera się przez `a[download]` z bloba.
- `ProductPage.tsx`: pole `ofertaPdf?: boolean` w `ProductData`, przycisk „Pobierz ofertę w PDF” (ciemnozielony, ikona `pobierz` w wersji białej) pod wierszami cennika, modal montowany przy stopce. Karta Pospay 2 ma `ofertaPdf: true`.
- `MIGRACJA_OFERTY_PDF.sql` — tabela `oferty_pdf` z RLS (insert/select dla anon, update tylko dopóki `numer is null`). **Jakub musi uruchomić w Supabase SQL Editor**, inaczej oferty nie trafiają do rejestru i mają numery z czasu.

**Iteracje po pierwszym podglądzie (uwagi Jakuba).** (1) „Za dużo czarnego” — ciemny pas nagłówka i czarne podsumowanie zamienione na biel; (2) logo: najpierw dołożone rejestratory.info obok TAKMA, po kolejnej uwadze („nasrane strasznie”) zostaje samo TAKMA; (3) boks „Warunki oferty” usunięty, płatność i realizacja zostają w drobnym druku; (4) abonament za terminal to teraz **drugi wiersz tabeli** pod urządzeniem (cena za sztukę i wartość z dopiskiem „miesięcznie”), a nie osobny kafel; (5) prowizje przeniesione obok podsumowania; (6) numery kroków w kółkach rozjeżdżały się w pionie — `lineHeight` na `Text` zamieniony na wyśrodkowany `View` (react-pdf nie centruje tekstu w kółku przez `lineHeight`); (7) numeracja na życzenie Jakuba: **`OF-RRRR-DDMMRRRR`** z daty wystawienia, więc kilka ofert z jednego dnia dzieli numer i w tabeli rejestru nie ma `UNIQUE` na `numer` (odpadła też druga runda zapytania do Supabase po `id`); (8) „wszystko się zlewa” — hierarchia kolorów: jedyny mocny akcent to ciemnozielony pas z kwotą brutto (limonkowa kwota) przyklejony do dołu boksu, pozostałe boksy białe z ramką `#cfdcd2`, jasnozielone tylko pas dat i uwagi zamawiającego, co drugi wiersz tabeli na `#f8faf8`, drobny druk bez ramki, numery kroków ciemnozielone.

**Co naprawdę zawiera abonament 39 zł** (sprawdzone na prośbę Jakuba u producenta i czterech sprzedawców: posnet.com.pl, kasy.pl, promokas.pl, intermarket.com.pl). Nasz opis „abonament dotyczy wynajmu terminala płatniczego” był za wąski. W kwocie mieszczą się: wynajem terminala (kupuje się samą drukarkę fiskalną) z dożywotnią gwarancją i serwisem, aplikacja sprzedażowa POS-UP na Androidzie ze zdalnymi aktualizacjami oraz karta SIM z transmisją danych — czyli **komunikacja z CRK**, a to obowiązek ustawowy kasy online, więc abonament nie jest dodatkiem do płatności kartą. Cennik eService ma też opłatę autoryzacyjną 0,03 zł przy PIN oraz Diners 3,20 % i UPI/JCB 0,99 %; **Jakub zdecydował, że 0,03 zł nie podajemy**. Poprawione w `pricing.note` na karcie i w `abonament.opis` w `produkty.ts`, a pod prowizjami w PDF doszło zdanie, że stawki VISA i MasterCard dotyczą kart wydanych w Polsce. Niepotwierdzone: jeden sprzedawca wymienia opłatę za raport do KAS 8 zł/mies. — do zapytania u operatora. Sklepy podają urządzenie po 1 899 zł netto, u nas 1 799 zł (nasza cena sprzedaży).

**Poprawki maila i drobiazgi (09.09).** Sprzedawca w PDF, stopce i mailu to teraz samo **„TAKMA”** (pełna nazwa zostaje w kontakcie, polityce prywatności i stopce serwisu, gdzie jest wymagana). Mail do klienta: wstęp skrócony do „W załączniku oferta nr …, ważna 30 dni” (bez „jest” i bez zdania o podstawie danych), tabela ograniczona do czterech wierszy — numer, produkt, wartość, ważność; nazwa jednostki i dane kontaktowe zostały tylko w powiadomieniu dla handlowca, rozbite na osobne wiersze. Kolorystyka rozjaśniona: pas tytułowy `#166534` zamiast `#0A1B12`, stopka jasna (`#f3f7f2`) i wyśrodkowana. Odmiana nazwy: helper `dopelniacz()` daje „dla Nadleśnictwa Wipsowo” w temacie i nagłówku (odmienia tylko pierwszy wyraz: Nadleśnictwo, Leśnictwo, Zakład, Dyrekcja, Regionalna; inne nazwy przechodzą bez zmian). Data ważności w mailu w formacie `DD.MM.RRRR`, jak w PDF. W modalu przepisany boks wstępu — mówi, co klient dostaje, a nie jak działa system.

**Produkcja wywalała 500 przy pierwszym deployu.** Log Vercela (`vercel logs --json`, bo zwykły widok ucina komunikat): `Cannot find module /var/task/node_modules/@react-pdf/renderer/node_modules/pdfkit/js/standard-fonts/Helvetica.cjs`. pdfkit ładuje standardowe fonty dynamicznie, więc tracing Next ich nie widzi; kopii pdfkit jest trzy (`@react-pdf/renderer`, `@react-pdf/font`, top-level). Naprawa w `outputFileTracingIncludes`: globy `./node_modules/pdfkit/js/**`, `./node_modules/@react-pdf/*/node_modules/pdfkit/js/**` i `./node_modules/**/fontkit/**`. Weryfikacja przed pushem bez czekania na deploy: `.next/server/app/api/oferta-pdf/route.js.nft.json` musi zawierać `standard-fonts/Helvetica` (po poprawce 16 plików) oraz DejaVu i logo.

**Modal przerobiony na jasny (09.09, po uwadze „za ciemne i klient musi scrollować”).** Ciemny panel `#0A1B12` zastąpiony białym oknem z zielonym paskiem nagłówka `#14532d`; pola w stylu formularzy z `/kontakt` (biel, ramka `stone-300`, focus emerald). Najważniejsza zmiana układu: `form` jest teraz `flex flex-col`, przewija się **tylko** środkowa część z polami (`flex-1 overflow-y-auto`), a stopka z przyciskiem ma `shrink-0` — wcześniej przewijała się razem z treścią i CTA wypadało poza ekran. Sprawdzone Playwrightem: na 1440 × 800 przycisk kończy się na 744 px, na 390 × 844 na 786 px, czyli widać go bez przewijania. Zgoda RODO i informacja o polach wymaganych zeszły do jednego drobnego wiersza nad stopką.

**Animowany przycisk w cenniku (09.09).** Zamiast płaskiego guzika `PrzyciskOferty` w `ProductPage.tsx`: pigułka z przesuwającym się gradientem zieleń → czerń i ledwo widocznymi drobinkami, które zapalają się, dryfują w górę i gasną. Droga: siedem bladych kropek („to nie są drobinki”) → dwadzieścia dwie z mocną poświatą → wygaszone o 20% → poziomice `ContourTexture` z panelu wdrożenia („słabo to widać”, potem „za mocno”) → powrót do drobinek przy kryciu warstwy 40% i słabszej poświacie. Pozycje wypisane na stałe, nie losowane przy renderze, żeby serwer i przeglądarka nie rozjechały się przy hydratacji. Wersja z poziomicami leży w commicie `de34619`, gdyby temat wrócił. Na koniec przycisk z wypukłego stał się wklęsły: zewnętrzny cień zamieniony na wewnętrzny (ciemny od góry, jasna krawędź u dołu), **na osobnej warstwie nad tłem i drobinkami** — `box-shadow: inset` na samym przycisku przykrywają leżące w nim `absolute inset-0`. Ten sam gradient i drobinki dostał **nagłówek okna z formularzem** (wcześniej płaska zieleń `#14532d`); pozycje drobinek wyprowadzone z `ProductPage` do `components/product/Drobinki.tsx`, żeby oba miejsca brały je z jednego źródła (wzór z przycisku, który przysłał Jakub, przemalowany na naszą paletę; bez białego kółka na końcu — odrzucone). Animacje w `globals.css` (`oferta-gradient`, `oferta-drobinka`), w komponencie zostają tylko pozycje i opóźnienia drobinek, żeby ruch nie był zsynchronizowany. **Pułapka:** przy `background-size: 200%` i pętli o pełną szerokość tła pierwszy i ostatni przystanek gradientu muszą mieć ten sam kolor, inaczej na styku powtórzenia widać pionowy szew (widoczny na pierwszym podejściu); z tego samego powodu kąt jest prosty, a nie ukośny. `prefers-reduced-motion` zatrzymuje obie animacje.

**Przebudowa prawej kolumny karty Pospay 2 (09.09).** Jakub: pierwszym krokiem na tej karcie ma być pobranie oferty, nie zapytanie. Nowa flaga `hideHeaderCta` w `ProductData` chowa skrót parametrów, parę przycisków „Zapytaj o produkt / Specyfikacja” oraz mobilne CTA przyklejone do dołu; z karty Pospay usunięte zostały też same `highlights`. **Flaga dotyczy wyłącznie tej karty** (pytany o zasięg, Jakub wybrał „tylko Posnet Pospay 2”) — pozostałe 79 kart zostaje bez zmian, sprawdzone na EM45. W bloku cennika kolejność to teraz cena → abonament → prowizje → nota → dopiero przycisk oferty, wydzielony własną sekcją na dole karty. Efekt: pod nazwą produktu od razu widać cennik, a przycisk zamyka blok zamiast rozbijać go w połowie.

**Abonament wyjęty z tabeli pozycji (09.09).** Jakub przy ofercie na 10 sztuk: wiersz z abonamentem w tabeli sugerował, że 390 zł miesięcznie wchodzi do sumy zakupu, a to dwa różne rodzaje kosztu (17 990 zł jednorazowo vs 39 zł miesięcznie za urządzenie). Tabela ma teraz tylko pozycje jednorazowe, a abonament razem z prowizjami stoi w bloku **„Opłata miesięczna”** obok podsumowania: kwota za komplet urządzeń, brutto, przelicznik na sztukę i opis, co zawiera. Pod kwotą podsumowania dopisek „Bez opłat miesięcznych wymienionych obok” (nagłówek „Zakup urządzeń — jednorazowo” Jakub kazał zdjąć). Prowizje trafiły najpierw do tego samego boksu, ale wyszło nieczytelnie („rozdziel na dwa boksy”), więc mają teraz własny pasek pod spodem: opis po lewej, trzy kafle po prawej — niższy niż wersja z kaflami pod tekstem, dlatego wszystko dalej mieści się na jednej stronie (sprawdzone dla 1 i 10 sztuk). Podsumowanie przestało się rozciągać do wysokości sąsiada (`alignItems: flex-start` zamiast `stretch`, `sumBody` bez `flex: 1`) — wcześniej ciemny pas z kwotą zjeżdżał na dół i zostawało wielkie puste pole.

**Runda poprawek 09.09 wieczorem.** (1) Nazwa nabywcy: klient wpisuje samo „Wipsowo”, więc route dokłada prefiks — `zNazwaJednostki()` daje „Nadleśnictwo Wipsowo”, chyba że nazwa już zaczyna się od określenia jednostki (RDLP, zakład, dyrekcja, leśnictwo…). (2) Opłata miesięczna pokazuje **zawsze stawkę za jedno urządzenie** (39 zł netto / 47,97 zł brutto), a nie iloczyn przez ilość. (3) Dopisek „Bez opłat miesięcznych wymienionych obok” usunięty. (4) Kafle prowizji powiększone (wartość 14 pt, wyższe pola, szerszy blok). (5) Cała treść rozluźniona, bo góra była ściśnięta, a na dole zostawało puste pole — większe odstępy między sekcjami i w tabeli.

**Pułapka przy rozluźnianiu:** po zwiększeniu odstępów drobny druk wypadał na drugą stronę, mimo że treść kończyła się na 750 pt z 808 dostępnych. `wrap={false}` na bloku, który nie mieści się w całości, przenosi go w komplecie; dwa punkty w osobnych `Text` liczyły się jako dwa wiersze. Zbite w jedno zdanie zmieściły się bez ruszania reszty. Kontrolnie sprawdzane zawsze dla 1 i 10 sztuk.
...
**Dół strony wypełniony (09.09).** Bez uwag zamawiającego oferta kończyła się w połowie strony. Doszedł pasek **„Jak zamówić”** (odpowiedź na ofertę albo telefon, potwierdzenie terminu i dokumenty do fiskalizacji), Drobny druk najpierw trafił do stopki, a potem wyleciał w całości — Jakub: skoro w dokumencie są kwoty brutto, zapis „ceny netto” tylko wprowadza zamęt. Zdanie o przeglądach ustawowych przeniesione do opisu pozycji, obok tego, co jest w cenie; pole `uwagi` w `produkty.ts` straciło odbiorcę i zostało usunięte. Uwagi zamawiającego przesunięte pod tabelę pozycji — w środku strony jest zapas, na dole go nie ma. Na koniec `body` dostało `flexGrow: 1` i `justifyContent: 'space-between'` — wolne miejsce rozkłada się między sekcje, więc treść sięga linii nad stopką zamiast urywać się w połowie strony; przy pełnej stronie nie ma czego rozkładać i układ zostaje bez zmian. Pasek pokazuje się tylko przy uwagach krótszych niż 120 znaków: gdy klient napisze dużo, dół jest już wypełniony, a pasek wypchnąłby treść na drugą stronę.

**Czego się nauczyłem o łamaniu stron w react-pdf** (kosztowało kilkanaście podejść): blok z `wrap={false}` przenosi się w całości, więc każdy taki element musi zmieścić się w tym, co zostało; `flex: 1` w wierszu potrafi rozciągnąć blok na całą pozostałą wysokość i wywołać przeniesienie — bezpieczniejsze są szerokości procentowe i `alignItems: 'flex-start'`; stopka `fixed` rezerwuje swoją wysokość na każdej stronie, więc realny limit treści jest o kilkadziesiąt punktów niższy niż `842 − paddingBottom`. Kontrola zawsze na sześciu wariantach: 1 i 10 sztuk × (bez uwag, krótkie, długie).
...
**Hero — dwa podejścia, drugie zostaje.** Pierwsze: gpt_image_2 z referencją `brother-l8900_1.png`, jasna kancelaria z lasem w oknie, leśniczy wyjmuje mapę z tacy (pierwsza para wyszła z wymyśloną naszywką „Lasy Państwowe” na rękawie, druga bez emblematów). Jakub po testówce: „lipa”, klimat ma być jak scena z kancelarią w filmie hero strony głównej. Stąd drugie podejście, tak jak przy Apple: **kadr wycięty z `public/hero-nadlesnictwo.mp4`** (sekundy 15,0–17,0, leśniczy z kartką przy urządzeniu wielofunkcyjnym, ciemne drewno, okno w świetle), `crop=1320:550:600:225` → 600 × 250, sześć klatek co 0,4 s tam i z powrotem, paleta 128 kolorów, **558 kB**. Pierwszy kadr przycinał głowę — okno kadru podniesione o 35 px. Ścieżka w mailu z `?v=2`, bo proxy Gmaila ma już pod tym adresem wersję z generatora. Wniosek na przyszłość: gdy film hero ma scenę z daną kategorią, brać ją z filmu, nie generować od nowa.

**Druga runda uwag (02.09, wieczór).** Z ciemnego bloku tytułowego wyleciał podtytuł „Pięć modeli w ofercie ZUP Łódź: mono od…”, a w bloku ceny podpis skrócony do „zakup przez Zakład Usługowo-Produkcyjny LP w Łodzi”. **Trzy nowe adresy w bazie odbiorców**: `handlowy@takma.com.pl`, `anita.lis@lodz.lasy.gov.pl`, `ewa.sasiak@lodz.lasy.gov.pl` — w nowym `newsletter/odbiorcy-dodatkowi.json`, scalanym w `lib/newsletter.ts` (pipeline) i w `scripts/send-newsletter.mjs` (ręcznie); `--list` pokazuje 605 odbiorców (było 602). Termin: Jakub wybrał **wtorek 08.09 8:30**. Bulk zaplanowany ręcznie 02.09 wieczorem przez `send-newsletter.mjs --send --send-at 2026-09-08T08:30:00+02:00` (605 odbiorców, ID w `scheduled-2026-09-08.json`), rezerwacja w `newsletter_sends` skryptem `zarezerwuj-wydanie.mjs`, więc poniedziałkowy przycisk „Zatwierdź” odpowie „Już zatwierdzone” i nie zdubluje. Uwaga: `handlowy@takma.com.pl` może odbić przez HostKarma (zob. pamięć `takma-mail-hostkarma`) — sprawdzić po wysyłce.

**Wieczór 02.09: wysyłka anulowana.** Zaraz po zakolejkowaniu 605 maili Jakub zatrzymał wysyłkę i kazał usunąć ceny — najpierw „za 1 375 zł netto” z leadu, potem wszystkie: „ceny w ogóle usuwamy, bo są po kliknięciu na stronie”. `cancel-scheduled.mjs` wycofał 592 maile, 13 zwróciło „Email is not scheduled” — sprawdzone przez `resend.emails.get`: wszystkie 13 mają status **suppressed** (adresy na liście blokad po wcześniejszych odbiciach), więc nie pójdą. Stare ID w `scheduled-2026-09-08.cancelled.json`, błędy w `scheduled-2026-09-08-cancel-errors.json`. Z maila wyleciały: cena w temacie i preheaderze, kolumna cen przy czterech kaflach (komórka usunięta, kafel zajmuje całą szerokość), kwota 3 103 zł w zielonym bloku (teraz „Zakup przez ZUP Łódź” jak w wydaniu o monitorach), „od 958 zł” przy drukarkach laserowych; podpisy zmienione na „Ceny urządzeń i tonerów są na kartach produktów na rejestratory.info”. Próbka wysłana. **Nowe planowanie dopiero po akceptacji Jakuba** — rezerwacja w `newsletter_sends` zostaje i blokuje poniedziałkowy przycisk, więc bez ręcznego `--send --send-at` wydanie nie pójdzie.

**04.09 — akceptacja i ponowne planowanie.** Po dwóch próbkach Jakub poprosił jeszcze o większy i niżej osadzony link „Zobacz modele →” na banerze Samsung (17 px, 34 px odstępu; `05fe78a`), a potem: „OK, zaplanuj na wtorek na 8:30”. Bulk zaplanowany ponownie przez `send-newsletter.mjs --send --send-at 2026-09-08T08:30:00+02:00` po sprawdzeniu, że produkcja serwuje wersję z ostatniego testu; nowe ID nadpisały `scheduled-2026-09-08.json`: **607 maili, 0 błędów** (baza urosła o dwa adresy osobiste z Supabase od 02.09; rezerwacja w `newsletter_sends` mówi 605 — to tylko licznik do wyświetlenia, blokada działa). Rezerwacja w `newsletter_sends` z 02.09 zostaje aktualna (ten sam plik i termin).

**Baner na dole: Samsung zamiast EM45** (uwaga Jakuba po drugiej próbce). Nowy `samsung-banner.jpg` (560 × 500) z gpt_image_2 z trzema referencjami 1:1 z `public/` — S25 Ultra z przodu, XCover 7 i A56 za nim, na mchu w nocnym lesie, światło w niebieskiej kolorystyce Samsunga (#1428A0) zamiast limonki; z dwóch wariantów wzięty ten, w którym telefony stoją w mchu, a nie wiszą nad nim. Logo: `samsung-logo-white.png` z simpleicons (curl `cdn.simpleicons.org/samsung/ffffff` → sharp → `-trim`), 15 px nad kickerem. Tło banera `#04091c`, akcenty `#7fa6ff`, tekst z opisów kategorii telefonów: XCover 7 IP68 i wymienna bateria, S25 Ultra S Pen i 200 Mpx; link na `/kategoria/telefony`. Po próbce Jakub: za dużo tekstu (nie wymieniać modeli, skoro jest link) i telefony słabo widoczne, nierówno. Druga wersja: tekst skrócony do „Smartfony / Galaxy dla nadleśnictw / Od wzmocnionego XCover 7 po S25 Ultra.”, grafika wygenerowana od nowa z promptem o równym rzędzie (ta sama wysokość, równe odstępy, na wprost, 85% wysokości kadru, jasne studyjne światło, rozmyte tło) — `?v=2` w ścieżce. Trzecia runda: Jakub — „logo Samsung wyżej” i „ta grafika to jak w Paincie, poprzednia była super, popraw tylko położenie telefonów”. Logo poszło do osobnego wiersza zagnieżdżonej tabeli (`valign="top"`, komórka 250 px), tekst wyśrodkowany w drugim wierszu. Grafika: **edycja pierwszej sceny** — poprzedni obraz banera wgrany jako pierwsza referencja obok trzech renderów, prompt „keep the scene exactly, change only the arrangement of the phones: one evenly spaced row, same angle, bottoms aligned, none hidden”. Zadziałało co do joty: ten sam las, mech i światło, telefony w równym rzędzie. `?v=3`. Wniosek: gdy scena jest dobra, a układ nie — podawać gotowy obraz jako referencję i edytować, zamiast generować od zera. Czwarta runda: blok tekstu podniesiony (drugi wiersz `valign="top"`, odstęp 22 px pod logo zamiast centrowania, które w Gmailu spychało tekst w dół) i **przejście tło → grafika**: lewe 130 px obrazu wtopione w `#04091c` gradientem (`gradient:` + `-compose over`), żeby krawędź kolumny nie wyglądała na uciętą; `?v=4`. Jakub: „brak zmian” — Gmail rozdał nadmiarową wysokość zagnieżdżonej tabeli na wiersz z logo i tekst dalej siedział nisko. Piąta wersja: **bez zagnieżdżonej tabeli**, logo i tekst w jednej komórce `valign="top"`, logo z `margin-bottom:24px`; gradient poszerzony ze 130 do 220 px; `?v=5`. Wniosek: w mailu nie centrować pionowo przez `height` tabeli z kilkoma wierszami — Gmail rozciąga wiersze po swojemu. Szósta wersja, bo gradient na kadrze z telefonami do samej krawędzi zaciemniał XCover („zmień sposób”): scena **dosztukowana w lewo** narzędziem `outpaint_image` Higgsfielda (job z równym rzędem → 16:9, 2752 × 1536), kadr `1720x1536+215+0` tak, że lewe ~220 px banera to sam las bez telefonów, a gradient 170 px kończy się przed pierwszym telefonem; `?v=6`. Jakub: „nadal jest odcięcie” — liniowy gradient przy krawędzi miał za małe krycie (jasne drobinki lasu prześwitywały przez 85%), a JPEG z podpróbkowaniem chrominancji przekłamywał granat. Siódma wersja: maska z **pełnym kryciem na pierwszych 40 px**, potem krzywa S (`-sigmoidal-contrast 6x50%`) do 165 px, czyli przed lewą krawędzią XCover (172 px), zapis `-sampling-factor 1x1 -quality 90`; piksel przy krawędzi = dokładnie `#04091c`; `?v=7`. Ósma: „wyostrz” — plik był 2× (560 px) i po skalowaniu z outpaintu wyszedł miękki; teraz **3× (840 × 750)** z `-unsharp 0x1.2+1.0+0.02`, maska przeskalowana (60 px pełne, S-krzywa do 248 px), atrybut `width="280"` bez zmian; `?v=8`. Dziewiąta próba (kolumna 320 px, długi gradient) — Jakub: „wróć do poprzedniego i zostawiamy”; commit cofnięty revertem (`7e71895`), zostaje wersja `?v=8` z kolumną 280 px.

**Poprawki po testówce (02.09, 17:53).** Pas u góry tylko „administratorowi SILP”; zdjęcie wiodące 260 → 182 px (o 30% mniejsze); lead uporządkowany („trzy monochromatyczne i dwa kolorowe”), bez zdania o TAKMA; w bloku ceny zamiast trzech kolumn dwie: „dostawa i serwis gwarancyjny TAKMA” (bez dwukropka) i „w ofercie również tonery oraz bębny” — Jakub odrzucił „tonery w tej samej ofercie ZUP” jako zły język, a notę o VAT jako zbędną.
...
Były dwa podejścia do wzmocnienia samego pomiaru: krawędzie na czerwono, potem grubsze białe z krawędzią głębokości doprowadzoną do pełnej długości (film urywa ją po 21 px, bo dalej zasłaniają ją kłody). Jakub obejrzał obie i wrócił do wersji pierwotnej — siatka zostaje taka, jaką rysuje film. Skrypty z tych prób leżą w scratchpadzie (`geometria.py` wykrywa krawędzie klatka po klatce, `rysuj.py` je nakłada), gdyby temat wrócił. Ścieżka w mailu ma `?v=4`, bo proxy Gmaila trzyma obrazki pod adresem. Kamera odjeżdża, więc współrzędne są wykrywane osobno w każdej klatce (`scratchpad/apple/geometria.py`): krawędzie pionowe to kolumny z największą liczbą jasnych, odbarwionych pikseli, dolna krawędź to prosta dopasowana do najniższych jasnych pikseli z odrzucaniem odstających. Bez tego czerwone linie odklejałyby się od stosu w trakcie pętli — bryła zwęża się z 709 do 682 px przez cztery klatki. Ścieżka w mailu dostała `?v=2`, bo proxy Gmaila trzyma obrazki pod starym adresem.
...
W mailu została ostatecznie tylko ta druga droga — Jakub skreślił akapit o mechanizmie skanera i kazał zawęzić lead wyłącznie do pomiaru LiDAR-em, bez opisywania ścieżki z aparatem. Rozróżnienie zostaje na kartach produktu, gdzie jest miejsce na kontekst. Nas interesuje droga bezreferencyjna — i to jest cały powód, dla którego urządzenia Apple są w ofercie. Wyleciało też zdanie, jakoby czoła kłód rozpoznawał obraz z aparatów: w rozwiązaniach LiDAR-owych czoła wychodzą z chmury punktów, więc przypisywanie tego aparatom było zgadywaniem.
